...
export function AuthScreen({ mode, onModeChange, onAuthenticated }: AuthScreenProps) {
  ...
  <div className="flex min-h-screen items-center justify-center bg-slate-100 px-4 dark:bg-slate-950">
    <div className="w-full max-w-md rounded-2xl bg-white p-8 shadow-xl dark:bg-slate-900 dark:shadow-black/20">
      ...
      <div className="mb-6 flex rounded-full bg-slate-100 p-1 text-sm font-medium text-slate-600 dark:bg-slate-800 dark:text-slate-300">
        <button
          type="button"
          onClick={() => onModeChange("login")}
          className={`flex-1 rounded-full px-3 py-2 transition ${
            isLogin ? "bg-white text-slate-900 shadow-sm dark:bg-slate-700 dark:text-white" : "dark:hover:text-slate-100"
          }`}
        >
          {t("auth.login")}
        </button>
        <button
          ...
          className={`flex-1 rounded-full px-3 py-2 transition ${
            isRegister ? "bg-white text-slate-900 shadow-sm dark:bg-slate-700 dark:text-white" : "dark:hover:text-slate-100"
          }`}
        >
          {t("auth.register")}
        </button>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {isLogin && (
          <div className="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
            <label className="inline-flex items-center gap-2">
              <input
                type="checkbox"
                className="h-3.5 w-3.5 rounded border-slate-300 dark:border-slate-600 dark:bg-slate-800 dark:text-sky-600"
                checked={rememberMe}
        ...
        <button
          type="submit"
          disabled={isLoading}
          className="mt-2 w-full rounded-lg bg-sky-600 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-sky-700 disabled:cursor-not-allowed disabled:bg-sky-400 dark:hover:bg-sky-500 dark:disabled:bg-sky-800 dark:disabled:text-slate-400"
        >
          {isLoading
